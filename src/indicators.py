...
def add_ema_indicators(
    ...
    enriched["ema_9"] = enriched["close"].ewm(span=fast_length, adjust=False).mean()
    enriched["ema_15"] = enriched["close"].ewm(span=slow_length, adjust=False).mean()
    enriched["ema_200"] = enriched["close"].ewm(span=trend_length, adjust=False).mean()
    ...
    enriched["atr_14"] = true_range.rolling(window=atr_length).mean()
