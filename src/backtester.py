...
def run_backtest(
    ...
    if (
        "long_signal" not in dataframe.columns
        or "short_signal" not in dataframe.columns
    ):
    ...
    for index, row in dataframe.iterrows():
        ...
        if index == len(dataframe) - 1:
            if open_position is not None:
                ...
                trade = {
                    ...
                    / max(open_position["entry_cost"], 1e-9),
        ...
        if open_position is None:
            if (
                int(row["long_signal"]) == 1
                and int(row["short_signal"]) != 1
            ):
                ...
                int(row["short_signal"]) == 1
                and int(row["long_signal"]) != 1
            ):
                entry_price = next_open
                units = balance / entry_price
                entry_fee = entry_price * units * fee_rate
