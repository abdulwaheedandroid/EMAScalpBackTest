...
class TestMVP(unittest.TestCase):
    ...
    def test_ema_crossover_detection(self) -> None:
        dataframe = pd.DataFrame(
            {
                "open_time": pd.date_range("2024-01-01", periods=30, freq="min"),
        ...
        result = generate_signals(with_indicators, use_ema_200_filter=False)

        self.assertIn("signal", result.columns)
        self.assertTrue(result["signal"].abs().sum() >= 1)

    def test_no_look_ahead_entry_behavior(self) -> None:
        dataframe = pd.DataFrame(
            {
                "open_time": pd.date_range("2024-01-01", periods=4, freq="min"),
        ...
        trades, _ = run_backtest(dataframe, initial_balance=1000.0, fee_rate=0.0)

        self.assertEqual(len(trades), 1)
        self.assertEqual(trades.iloc[0]["entry_time"], dataframe["open_time"].iloc[2])
        self.assertEqual(trades.iloc[0]["entry_price"], 100.0)

    def test_profit_calculation(self) -> None:
        dataframe = pd.DataFrame(
            {
                "open_time": pd.date_range("2024-01-01", periods=3, freq="min"),
                "open": [100, 100, 110],
                "high": [100, 110, 110],
                "low": [100, 90, 110],
                "close": [100, 110, 110],
        ...
        self.assertEqual(len(trades), 1)
        self.assertAlmostEqual(trades.iloc[0]["profit_loss"], 10.0)
